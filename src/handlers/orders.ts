...
const addProductToUser = async (req: Request, res: Response) => {
    try {
        store.addProductToUser(req.params.userId, parseInt(req.body.quantity), req.body.productId);
...
const deleteProductFromOrder = async (req: Request, res: Response) => {
    try {
        const order = store.deleteProductFromOrder(req.params.orderId, req.params.productId);
        res.status(200).json(order);
    } catch(err) {
        res.json(err);
    }
